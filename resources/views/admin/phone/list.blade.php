...
<body>
    ...
    <ul>
        @foreach($phone_in_view as $item)
            <li>
                {{$item -> name}}
                <img src="{{$item -> images}}" alt="" style="width: 100px; border-radius: 50%">
                <a href="/admin/phone/edit/{{$item -> id}}">Edit</a>&nbsp;&nbsp;
                <a href="/admin/phone/delete/{{$item -> id}}">Delete</a>&nbsp;&nbsp;
        ...
        for (var i = 0; i < listDeleteButton.length; i++) {
            listDeleteButton[i].onclick = function () {
                if(confirm('Are you sure ?')){
                    ...
                    xhttp.open("POST", "/admin/phone/destroy/" + currentId, true);
